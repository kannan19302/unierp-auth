...
describe("Purpose-scoped tokens", () => {
  it("accepts a session token only as a session", () => {
    ...
    expect(verifyTypedToken(token, TOKEN_TYPE.SESSION)).toMatchObject({
      userId: "u1",
    });
  ...
  it("rejects a reset token when a session is required", () => {
    const reset = signTypedToken(
      TOKEN_TYPE.PASSWORD_RESET,
      { userId: "u1" },
      "1h",
    );
    expect(verifyTypedToken(reset, TOKEN_TYPE.PASSWORD_RESET)).toMatchObject({
      userId: "u1",
    });
  ...
  it("rejects an MFA-challenge token as a session", () => {
    const challenge = signTypedToken(
      TOKEN_TYPE.MFA_CHALLENGE,
      { userId: "u1" },
      "5m",
    );
    expect(verifyTypedToken(challenge, TOKEN_TYPE.SESSION)).toBeNull();
    expect(verifyTypedToken(challenge, TOKEN_TYPE.MFA_CHALLENGE)).toMatchObject(
      { userId: "u1" },
    );
  });

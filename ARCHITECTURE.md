# Architecture Specification: UniERP Authentication & Security Primitives (`auth`)

- **Layer**: Layer L1 (Foundation)
- **Package Identity**: `@kannan19302/auth`
...
Core authentication primitives, token parsing, RBAC/ABAC verification, and permission bitsets.
...
```mermaid
graph LR
  Callers["Allowed Inbound Callers<br/>L2-L5 (Runtime, Service, Presentation, Clients)"] --> Repo["<b>auth (L1)</b><br/>UniERP Authentication & Security Primitives"]
  Repo --> Outbound["Allowed Outbound Dependencies<br/>@kannan19302/contracts (L0)"]
  
  Forbidden["Strictly Forbidden<br/>Layers L2-L7, Database ORM directly, HTTP servers directly"] -.-x Repo
...
### Boundary Contract
- **Allowed Inbound Consumers**: L2-L5 (Runtime, Service, Presentation, Clients)
- **Allowed Outbound Dependencies**: @kannan19302/contracts (L0)
- **Strictly Forbidden Dependencies**:
  - ❌ Layers L2-L7
  - ❌ Database ORM directly
...
- **Core Runtime & Languages**: TypeScript, Jose, Vitest
- **Primary Interface**: `@kannan19302/auth`
...
- **Project Skill**: [`.agents/skills/auth-security-standards/SKILL.md`](.agents/skills/auth-security-standards/SKILL.md)
